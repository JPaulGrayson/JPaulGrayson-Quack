...
import { VALID_STATUSES, STATUS_TRANSITIONS } from '../types/index.js';
export function createQuackRouter(options) {
    const { store } = options;
    const router = Router();
    ...
    router.get('/inbox/:name', async (req, res) => {
    ...
    router.get('/inbox/:parent/:child', async (req, res) => {
        const inbox = `${req.params.parent}/${req.params.child}`;
        const includeRead = req.query.includeRead === 'true';
        const messages = await store.checkInbox(inbox, includeRead);
        res.json({ inbox, messages, count: messages.length });
    });
    router.get('/inbox/:parent/:child/:subchild', async (req, res) => {
    ...
    router.post('/status/:id', async (req, res) => {
        ...
        const message = await store.updateMessageStatus(req.params.id, status);
        res.json({ success: true, message });
    });
